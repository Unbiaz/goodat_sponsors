...
class JobsController extends AppController
{
    ...
    public function index()
    {
        ...
        //if(!$this->isSubcriber() && !$this->isAdmin())$this->paginate['limit']=20;
        
        if($this->isAdmin()) 
             $jobs = $this->paginate($this->Jobs);
        else $jobs = $this->paginate($this->Jobs->find('all')->where(['expirated >='=>date('Y-m-d H:i:s')]));
    ...
    public function area($id)
    {
        ...
        else $jobs = $this->paginate($this->Jobs->find('all')->where(['area_id'=>$id, 'expirated >='=>date('Y-m-d H:i:s')]));
    ...
    public function location($id)
    {
        ...
        else $jobs = $this->paginate($this->Jobs->find('all')->where(['location_id'=>$id, 'expirated >='=>date('Y-m-d H:i:s')]));
