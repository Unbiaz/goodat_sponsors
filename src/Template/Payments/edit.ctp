...
<h4 class="uk-margin-small-bottom uk-margin-large-top uk-text-center txt-green">Edit Payment</h4>
...
<div class="uk-panel uk-padding-small bg-green">
    <?= $this->Form->create($payment, ['class' => 'uk-grid-small uk-grid-match uk-child-width-1-2@s uk-child-width-1-3@m', 'uk-grid'=>'']) ?>
        ...
        <?= $this->Form->control('amount', ['type'=>'text', 'class' => 'uk-input uk-form-small', 'label' => 'Amount in £', 'placeholder' => 'Enter a amount']); ?>

        <div class="">
            <p class="uk-margin-remove">Valid to</p>
            <div class="uk-inline uk-width-1-1">
                <span class="uk-form-icon uk-form-small uk-padding-remove" uk-icon="icon: calendar"></span>
                <?= $this->Form->input('validTo', ['type'=>'text', 'id'=>'datetimepicker', 'class' => 'uk-input uk-form-small', 'label'=>'']); ?>
            </div>
        </div>

        <?= $this->Form->control('user_id', ['type'=>'select', 'class'=>'uk-select uk-form-small chosen-select', 'options' => $users, 'label' => 'Select username']); ?>
