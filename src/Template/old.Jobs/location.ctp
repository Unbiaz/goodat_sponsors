...
<div class="uk-float-right uk-margin-bottom">
  ...
  <div class="uk-button-group">
    <div class="uk-inline">
          <button class="uk-button uk-button-primary bg-green uk-button-small uk-margin-small-top" type="button"><?= h($location->name) ?> <i class="uk-margin-small-left" uk-icon="icon:  triangle-down"></i></button>
...
<div class="uk-position-relative">
    <ul class="txt-black uk-grid-small uk-child-width-1-2@s uk-grid-match" uk-grid>
    ...
    <?php if(!$isSubcriber): ?>
      <div class="callto-subscribe uk-text-center">
      ...
      <table class="uk-table uk-table-small uk-table-hover uk-table-striped">
        ...
        <tbody>
            ...
            <tr>
                <td><?= h($job->id) ?></td>
                <td><?= h($job->title) ?></td>
                <td><?= $this->Html->link(h($job->company['name_company']), ['controller'=>'Companies', 'action'=>'view', $job->company['id_cmpny']]) ?></td>
